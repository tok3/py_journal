...
class Admin_Categories extends Admin_Controller
{
	 ...
	 * - load the journal_categories model.
	 * - load the categories and journal language files.
	...
	public function index()
	{
		...
		$total_rows = $this->journal_categories_m->count_all();
		$pagination = create_pagination('admin/journal/categories/index', $total_rows, Settings::get('records_per_page'), 5);

		// Using this data, get the relevant results
		$categories = $this->journal_categories_m
								->order_by('title')
	...
	public function create()
	{
		...
		{
			if ($id = $this->journal_categories_m->insert($this->input->post()))
			{
				// Fire an event. A new journal category has been created.
				Events::trigger('journal_category_created', $id);
			...
			redirect('admin/journal/categories');
		}
	...
	{
		// Get the category
		$category = $this->journal_categories_m->get($id);

		// ID specified?
		$category or redirect('admin/journal/categories/index');
		...
		if ($this->form_validation->run())
		{
			$this->journal_categories_m->update($id, $this->input->post())
			...
			// Fire an event. A journal category is being updated.
			Events::trigger('journal_category_updated', $id);

			redirect('admin/journal/categories/index');
		}
	...
	public function delete($id = 0)
	{
		...
		if (!empty($id_array))
		{
			...
			{
				if ($this->journal_categories_m->delete($id))
				{
					$deleted++;
					$deleted_ids[] = $id;
				}
			...
			Events::trigger('journal_category_deleted', $deleted_ids);
		}
		else
		{
	...
	{
		if ($this->journal_categories_m->check_title($title, $this->input->post('id')))
		{
	...
	{
		if ($this->journal_categories_m->check_slug($slug, $this->input->post('id')))
		{
	...
	public function create_ajax()
	{
		...
		if ($this->form_validation->run())
		{
			$id = $this->journal_categories_m->insert_ajax($this->input->post());
