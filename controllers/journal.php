 ...
 */
class Journal extends Public_Controller
{
	public $stream;
	 ...
	 * - load the journal language file.
	 */
	public function __construct()
	{
		...
		$cates = $this->db->get('journal_categories')->result_array();
		...
		// Get journal stream. We use this to set the template
		// stream throughout the journal module.
		$this->stream = $this->streams_m->get_stream('journal', true, 'journals');
	}

	/**
	 * Index
	 *
	 * List out the journal posts.
	 *
	 * URIs such as `journal/page/x` also route here.
	 */
	public function index()
	{
		...
		$this->row_m->sql['select'][] = "(SELECT COUNT(id) FROM ".
			$this->db->protect_identifiers('comments', true)." WHERE module='journal'
			AND is_active='1' AND entry_key='journal:post' AND entry_plural='journal:posts'
			AND entry_id=".$this->db->protect_identifiers('journal.id', true).") as `comment_count`";

		// Get the latest journal posts
$posts = $this->streams->entries->get_entries(array(
	...
	'where'			=> "`status` = 'live'",
	'paginate'		=> 'yes',
	'pag_base'		=> site_url('journal/page'),
	'pag_segment'   => 3
	));
...
->set_breadcrumb(lang('journal:journal_title'))
->set_metadata('og:title', $this->module_details['name'], 'og')
->set_metadata('og:type', 'journal', 'og')
...
->build('posts');
	...
	{
		$slug or redirect('journal');

		// Get category data
		$category = $this->journal_categories_m->get_by('slug', $slug) OR show_404();

		// Get the journal posts
		$params = array(
			'stream'		=> 'journal',
			'namespace'		=> 'journals',
			'limit'			=> Settings::get('records_per_page'),
		...
		->set_breadcrumb(lang('journal:journal_title'), 'journal')
	...
	public function archive($year = null, $month = '01')
	{
		...
		$month_year = format_date($month_date->format('U'), lang('journal:archive_date_format'));
		...
		$this->template
		->title($month_year, lang('journal:archive_title'), lang('journal:journal_title'))
		->set_metadata('description', $month_year.'. '.$meta['description'])
		->set_metadata('keywords', $month_year.', '.$meta['keywords'])
		->set_breadcrumb(lang('journal:journal_title'), 'journal')
		->set_breadcrumb(lang('journal:archive_title').': '.format_date($month_date->format('U'), lang('journal:archive_date_format')))
		->set('pagination', $posts['pagination'])
		->set_stream($this->stream->stream_slug, $this->stream->stream_namespace)
		->set('posts', $posts['entries'])
	 ...
	 * View a post
	 *
	 * @param string $slug The slug of the journal post.
	 */
	...
	public function preview($hash = '')
	{
		...
		if ($post['status'] === 'live')
		{
			redirect('journal/'.date('Y/m', $post['created_on']).'/'.$post['slug']);
		}
	 ...
	 * Tagged Posts
	 *
	 * Displays journal posts tagged with a
	...
	public function tagged($tag = '')
	{
		...
		$tag = rawurldecode($tag) or redirect('journal');
		...
		// Maybe the entire journal moduel should be replaced with stream
		...
		$this->db->protect_identifiers('journal.keywords', true);
		...
		// Build the page
		$this->template
		->title($this->module_details['name'], lang('journal:tagged_label').': '.$name)
		->set_metadata('description', lang('journal:tagged_label').': '.$name.'. '.$meta['description'])
		->set_metadata('keywords', $name)
		->set_breadcrumb(lang('journal:journal_title'), 'journal')
		->set_breadcrumb(lang('journal:tagged_label').': '.$name)
		->set('pagination', $posts['pagination'])
		->set_stream($this->stream->stream_slug, $this->stream->stream_namespace)
		->set('posts', $posts['entries'])
		->set('tag', $tag)
	...
	// if it uses markdown then display the parsed version
		if ($post['type_body'] === 'markdown')
		{
			$post['body'] = $post['parsed_body'];
		}
		if ($post['type_teaser'] === 'markdown')
		{
			$post['teaser'] = $post['parsed_teaser'];
		}
		...
		$post['url'] = site_url('journal/'.date('Y/m', $post['created_on']).'/'.$post['slug']);

		// What is the preview? If there is a field called intro,
		// we will use that, otherwise we will cut down the journal post itself.
	...
	private function _single_view($post)
	{
		...
		$this->template->set_breadcrumb(lang('journal:journal_title'), 'journal');

		if ($post['category_id'] > 0)
		{
			// Get the category. We'll just do it ourselves
			// since we need an array.
			if ($category = $this->db->limit(1)->where('id', $post['category_id'])->get('journal_categories')->row_array())
			{
				$this->template->set_breadcrumb($category['title'], 'journal/category/'.$category['slug']);
