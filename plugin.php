 ...
 * @package  PyroCMS\Core\Modules\Journal\Plugins
 */
class Plugin_Journal extends Plugin
{
	...
	public $description = array(
		'en' => 'A plugin to display information such as journal categories and posts.',
            'fa' => 'یک پلاگین برای نمایش اطلاعاتی مانند مجموعه های بلاگ و پست ها',
        'fr' => 'Un plugin permettant d\'afficher des informations comme les catégories et articles du journal.'
	);
		...
		$info = array(
			'posts' => array(
				'description' => array(// a single sentence to explain the purpose of this method
					'en' => 'Display journal posts optionally filtering them by category.',
                    'fr' => 'Permet d\'afficher des articles de journal en les filtrants par catégorie.'
				),
			...
			'categories' => array(
				'description' => array(
					'en' => 'List journal categories.',
                    'fr' => 'Lister les catégories du journal'
				),
				'single' => false,
				'double' => true,
			...
			'count_posts' => array(
				'description' => array(
					'en' => 'Count journal posts that meet the conditions specified.',
                    'fr' => 'Permet de compter les articles de journal qui remplissent certaines conditions spécifiées.'
				),
				'single' => true,
				'double' => false,
				'variables' => '',
			...
			'tags' => array(
				'description' => array(
					'en' => 'Retrieve all tags that have been applied to journal posts.',
	 ...
	 * Journal List
	 *
	 * Creates a list of journal posts. Takes all of the parameters
	 * available to streams, sans stream, where, and namespace.
	 *
	 * Usage:
	 * {{ journal:posts limit="5" }}
	 *		<h2>{{ title }}</h2>
	 * {{ /journal:posts }}
	 *
	 * @param	array
	...
	public function posts()
	{
		...
		if ($category_string = $this->attribute('category'))
		{
			...
			{
				$cate_filter_by[] = '`'.$this->db->dbprefix('journal_categories').'`.`'.(is_numeric($category) ? 'id' : 'slug').'` = \''.$category."'";
			}
		...
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('journal_categories.title', true)." as 'category_title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('journal_categories.slug', true)." as 'category_slug'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('journal_categories.title', true)." as 'category||title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('journal_categories.slug', true)." as 'category||slug'";
		$this->row_m->sql['join'][] = 'LEFT JOIN '.$this->db->protect_identifiers('journal_categories', true).' ON '.$this->db->protect_identifiers('journal_categories.id', true).' = '.$this->db->protect_identifiers('journal.category_id', true);
		...
		// Return our content.	
		return $this->streams->parse->parse_tag_content($this->content(), $posts, 'journal', 'journals', $loop);
	}

	/**
	 * Categories
	 *
	 * Creates a list of journal categories
	 *
	 * Usage:
	 * {{ journal:categories order-by="title" limit="5" }}
	...
	public function categories()
	{
		...
		$categories = $this->db
			...
			->limit($limit)
			->get('journal_categories')
			->result();

		foreach ($categories as &$category)
		{
			$category->url = site_url('journal/category/'.$category->slug);
		}
	 ...
	 *
	 * Usage:
	 * {{ journal:count_posts author_id="1" }}
	 *
	 ...
	 * Tag/Keyword List
	 *
	 * Create a list of journal keywords/tags
	 *
	 * Usage:
	 * {{ journal:tags limit="10" }}
	...
	public function tags()
	{
		...
		$posts = $this->db->select('keywords')->get('journal')->result();
