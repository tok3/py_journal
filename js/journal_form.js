(function ($) {
	$(function () {
		...
		pyro.generate_slug('#journal-content-tab input[name="title"]', '#journal-content-tab input[name="slug"]');
		...
		$(document.getElementById('journal-options-tab')).find('ul').find('li').first().find('a').colorbox({
			srollable: false,
			innerWidth: 600,
			innerHeight: 280,
			href: SITE_URL + 'admin/journal/categories/create_ajax',
			onComplete: function () {
				$.colorbox.resize();
				...
				$form_categories.live('submit', function (e) {
					...
					$.ajax({
						url: SITE_URL + 'admin/journal/categories/create_ajax',
						type: "POST",
						data: form_data,
						success: function (obj) {
							...
							if (obj.status == 'ok') {
								...
								$(document.getElementById('journal-options-tab')).find('li').first().find('span').html(obj.title);
